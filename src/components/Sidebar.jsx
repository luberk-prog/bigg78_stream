...
const SidebarIcon = ({ children, to, active }) => (
  <Link
    to={to}
...
export default function Sidebar() {
  ...
  return (
    <aside className="fixed left-6 top-1/2 -translate-y-1/2 z-50 flex flex-col gap-4 p-2 rounded-[2.5rem] glass-dark border border-white/10 shadow-2xl">
      ...
      <SidebarIcon to="/search" active={pathname === '/search'}>
        <svg className="w-6 h-6 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
        </svg>
      </SidebarIcon>

      <SidebarIcon to="/favorites" active={pathname === '/favorites'}>
        <svg className="w-6 h-6 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <SidebarIcon to="/settings" active={pathname === '/settings'}>
        <svg className="w-6 h-6 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
